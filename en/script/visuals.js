...
$(document).ready(function() {
    ...
    //Create the column numbers on top of the game board
    for (let i = 1; i <= configuration.board_size; i++)
        board_container.append($("<div/>", {text: i, class: "header_number"}));
...
$(document).keydown(function (e) {
	...
	if (is_board_focused) {
		...
		else if (e.keyCode === 46 && !is_cell_committed[row_index][col_index]) {
		...
		else if (e.keyCode >= 65 && e.keyCode <= 90 && !is_cell_committed[row_index][col_index]) {
            focused_element_DOM.attr("letter", e.keyCode);
            setBackgroundImage(focused_element_DOM, getLetterImage(e.keyCode));
	...
	for (let row_index = 0; row_index < configuration.board_size; row_index++)
        for (let col_index = 0; col_index < configuration.board_size; col_index++)
            if (is_cell_changed[row_index][col_index]) {
                ...
                let ascii_code = parseInt(getCellDOM(row_index, col_index).attr("letter"));
